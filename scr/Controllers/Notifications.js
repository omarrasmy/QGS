...
exports.PushNotification = async (req, res) => {
  ...
  for (var i =0 ; i<Notif.length;i++){
    id=Notif[i].tokens.find(usertoken => usertoken == token)
    if(id){
        x=Notif[i].tokens.indexOf(token)
        NotifcationContent=Notif[i]
        Notif.tokens.splice(x,1)
        await Notif[i].save()
        break;
    }
  }
  if(id){
      if(NotifcationContent.pushFlag === false){
        ...
        // Create payload
        const payload = JSON.stringify({ title: "Quizy",Description:Notification.Description });
